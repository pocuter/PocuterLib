...
#include "include/hal/esp32-c3/pwmSound/esp32c3_timer_group_struct.h"
#endif
...
static ringbuf_handle_t *rb_create(uint32_t size)
{
    ...
    do {
        bool _success =
            (
                (rb             = (ringbuf_handle_t *)heap_caps_malloc(sizeof(ringbuf_handle_t), MALLOC_CAP_8BIT | MALLOC_CAP_INTERNAL)) &&
...
esp_err_t pwm_audio_init(const pwm_audio_config_t *cfg)
{
    ...
    handle = (pwm_audio_data_t*)heap_caps_malloc(sizeof(pwm_audio_data_t), MALLOC_CAP_8BIT | MALLOC_CAP_INTERNAL);
    ...
    res = pwm_audio_set_param(16000, LEDC_TIMER_8_BIT, 2);
